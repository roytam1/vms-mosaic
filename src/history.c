...
mo_status mo_free_node(mo_node *node)
{
  ...
  if (node->cached_widgets)
      HTMLFreeWidgetInfo(node->cached_widgets);
  if (node->cached_forms)
      HTMLFreeFormInfo(node->cached_forms);
  if (node->image_file) {
      remove(node->image_file);
      free(node->image_file);
  }
  ...
  if (node->frames) {
      ...
      while (frame) {
	  /* Deaccess its text in cache */
	  mo_deaccess_cached_frame_data(frame->url);
...
static mo_status mo_add_node_to_history(mo_window *win, mo_node *node)
{
  ...
  if (win->history_list) {
      XmString xmstr = XmxMakeXmstrFromString(displayURLs ?
			                      node->url : node->title);
...
static char *mo_grok_alternate_title(char *url, char *ref, mo_node *node)
{
  ...
  if (!strncmp(url, "file:", 5)) {
      ...
      if (!strncmp(url, "file:///", 8)) {
          ...
          goto done;
      } else if (!strncmp(url, "file://localhost/", 17)) {
          /* It's a local file. */
          foo1 = url + 16;
  ...
  if (!win->history_win) {
	...
	"~Shift ~Ctrl ~Meta ~Alt <Btn2Down>: ListKbdSelectAll() ListBeginSelect() \n\
	 ~Shift ~Ctrl ~Meta ~Alt <Btn2Up>:   ListEndSelect() ListKbdActivate()";
      ...
      XmxAddClue(win->history_list, "Double click on entry to visit it");
