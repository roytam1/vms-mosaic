...
mo_status mo_setup_global_history (void)
{
  ...
  filename = (char *)malloc((strlen(home) + strlen(history_file) + 8) *
	     		    sizeof(char));
#ifndef VMS
  sprintf (filename, "%s/%s\0", home, history_file);
#else
  sprintf (filename, "%s%s\0", home, history_file);
#endif /* VMS, BSN */
...
#ifndef VMS
      sprintf(tmpnam, "%s/%s\0", home, default_filename);
#else
      sprintf(tmpnam, "%s%s\0", home, default_filename);
#endif /* VMS, BSN */
...
{
  mo_frame *frame, *start;
  HTMLWidget hw;
  int num = 0;

  start = frame = (mo_frame *)malloc(sizeof(mo_frame));
  while (win) {
      frame->next = NULL;
