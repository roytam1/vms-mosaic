...
#else
extern FcChar16 *langBankNumbers;
extern FcCharLeaf	*langBankLeaves;
extern int *langBankLeafIdx;
extern void FcInitLangCharSets();
#endif
...
#else
static FcCharLeafEnt **FcCharLeafBlocks = NULL;
static int FcCharLeafBlockCount = 0;
#endif
...
FcCharLeafEntCreate (void)
{
	...
	newBlocks = g_realloc (FcCharLeafBlocks, FcCharLeafBlockCount * sizeof (FcCharLeafEnt *));
	if (!newBlocks)
	    return 0;
...
FcCharSetEnsureBank (int bi)
{
	...
	cs = g_realloc(charsets, sizeof(FcCharSet*) * new_count);
	if (!cs) return 0;
	n = g_realloc(numbers, sizeof(FcChar16*) * new_count);
	if (!n) return 0;
	lvs = g_realloc(leaves, sizeof(FcCharLeaf*) * new_count);
	if (!lvs) return 0;
	lvi = g_realloc(leaf_idx, sizeof(int*) * new_count);
	if (!lvi) return 0;
