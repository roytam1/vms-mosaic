		 ...
		 FcConfig	*config)
{
    ...
    int			i;
#ifdef VMS
    extern char *VMSopendir_dirname;
#endif
	...
	if (FcDirCacheValid (dir) && FcDirCacheHasCurrentArch (dir) &&
...
#ifdef VMS
    d = VMSopendir ((char *) dir);
    file = (FcChar8 *) malloc(strlen(VMSopendir_dirname) + 1 + FC_MAX_FILE_LEN + 1);
    if (!file)
	return FcFalse;
    strcpy ((char *) file, VMSopendir_dirname);
#else
    d = opendir ((char *) dir);
...
#else
	FcGlobalCacheUpdate (cache, VMSopendir_dirname, tmpSet);
#endif
