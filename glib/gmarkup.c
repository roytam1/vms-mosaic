      ...
      if (*p == ';')
      ...
      else
        {
          ...
          set_unescape_error (ucontext->context, error,
                              p, ucontext->text_end,
                              G_MARKUP_ERROR_PARSE,
                              _("Character '%s' is not valid at the start of an entity name; the & character begins an entity; if this ampersand isn't supposed to be an entity, escape it as &amp;"),
                              utf8_str (p, buf));
        }
    }
  else
      ...
      if (*p == ';')
      ...
      else if (!is_name_char (p))
        {
          ...
          set_unescape_error (ucontext->context, error,
                              p, ucontext->text_end,
                              G_MARKUP_ERROR_PARSE,
                              _("Character '%s' is not valid inside an entity name"),
                              utf8_str (p, ubuf));
          break;
        }
	  ...
	  gint len = p - ucontext->entity_start;
      ...
      else
        {
          set_unescape_error (ucontext->context, error,
                              ...
                              G_MARKUP_ERROR_PARSE,
                              _("Entity did not end with a semicolon; most likely you used an ampersand character without intending to start an entity - escape ampersand as &amp;"));
        }
    }
#undef MAX_ENT_LEN
      ...
      if (start != p)
          ...
          if (end != p || errno != 0)
            {
              set_unescape_error (ucontext->context, error,
                                  ...
                                  G_MARKUP_ERROR_PARSE,
                                  _("Failed to parse '%-.*s', which should have been a digit inside a character reference (&#234; for example) - perhaps the digit is too large"),
                                  p - start, start);
            }
          else
            {
      ...
      else
        {
          set_unescape_error (ucontext->context, error,
                              start, ucontext->text_end,
                              G_MARKUP_ERROR_PARSE,
                              _("Empty character reference; should include a digit such as &#454;"));
        }
    }
  else
    {
      set_unescape_error (ucontext->context, error,
                          start, ucontext->text_end,
                          G_MARKUP_ERROR_PARSE,
                          _("Character reference did not end with a semicolon; most likely you used an ampersand character without intending to start an entity - escape ampersand as &amp;"));
    }
	      ...
	      context->balance = 1;
          ...
          else
            {
              ...
              set_error (context,
                         error,
                         G_MARKUP_ERROR_PARSE,
                         _("'%s' is not a valid character following a '<' character; it may not begin an element name"),
                ...
                if (*context->iter == '>')
                ...
                else
                  {
                    ...
                    set_error (context,
                               error,
                               G_MARKUP_ERROR_PARSE,
                               _("Odd character '%s', expected a '>' character to end the start tag of element '%s'"),
                               utf8_str (context->iter, buf),
              ...
              if (*context->iter == '=')
              ...
              else
                {
                  ...
                  set_error (context,
                             error,
                             G_MARKUP_ERROR_PARSE,
                             _("Odd character '%s', expected a '=' after attribute name '%s' of element '%s'"),
                             utf8_str (context->iter, buf),
              ...
              if (*context->iter == '/')
              ...
              else
                {
                  ...
                  set_error (context,
                             error,
                             G_MARKUP_ERROR_PARSE,
                             _("Odd character '%s', expected a '>' or '/' character to end the start tag of element '%s', or optionally an attribute; perhaps you used an invalid character in an attribute name"),
                             utf8_str (context->iter, buf),
                             current_element (context));
                }
	  ...
	  if (context->iter != context->current_text_end)
	      ...
	      else
		{
		  ...
		  set_error (context,
			     error,
			     G_MARKUP_ERROR_PARSE,
		     _("Odd character '%s', expected an open quote mark after the equals sign when giving value for attribute '%s' of element '%s'"),
			     utf8_str (context->iter, buf),
			     current_attribute (context),
			     current_element (context));
		}
          ...
          if (is_name_start_char (context->iter))
          ...
          else
            {
              ...
              set_error (context,
                         error,
                         G_MARKUP_ERROR_PARSE,
                         _("'%s' is not a valid character following the characters '</'; '%s' may not begin an element name"),
                         utf8_str (context->iter, buf),
                         utf8_str (context->iter, buf));
            }
          break;
	  ...
	  if (context->iter != context->current_text_end)
	      ...
	      if (*context->iter != '>')
		{
		  ...
		  set_error (context,
			     error,
			     G_MARKUP_ERROR_PARSE,
			     _("'%s' is not a valid character following the close element name '%s'; the allowed character is '>'"),
			     utf8_str (context->iter, buf),
			     close_name);
		}
	      else if (context->tag_stack == NULL)
		{
		  set_error (context,
			     error,
			     G_MARKUP_ERROR_PARSE,
			     _("Element '%s' was closed, no element is currently open"),
			     close_name);
		}
	      else if (strcmp (close_name, current_element (context)) != 0)
		{
		  set_error (context,
			     error,
			     G_MARKUP_ERROR_PARSE,
			     _("Element '%s' was closed, but the currently open element is '%s'"),
			     close_name,
			     current_element (context));
		}
	      else
                                  ...
                                  GError             **error)
{
  ...
  switch (context->state)
      ...
      if (context->tag_stack != NULL)
        {
          ...
          set_error (context, error, G_MARKUP_ERROR_PARSE,
                     _("Document ended unexpectedly with elements still open - '%s' was the last element opened"),
                     current_element (context));
        }
      break;
    ...
    case STATE_AFTER_ELISION_SLASH:
      set_error (context, error, G_MARKUP_ERROR_PARSE,
                 _("Document ended unexpectedly, expected to see a close angle bracket ending the tag <%s/>"), current_element (context));
      break;
    ...
    case STATE_BETWEEN_ATTRIBUTES:
      set_error (context, error, G_MARKUP_ERROR_PARSE,
                 _("Document ended unexpectedly inside an element-opening tag."));
      break;

    case STATE_AFTER_ATTRIBUTE_EQUALS_SIGN:
      set_error (context, error, G_MARKUP_ERROR_PARSE,
                 _("Document ended unexpectedly after the equals sign following an attribute name; no attribute value"));
      break;

    case STATE_INSIDE_ATTRIBUTE_VALUE_SQ:
    case STATE_INSIDE_ATTRIBUTE_VALUE_DQ:
      set_error (context, error, G_MARKUP_ERROR_PARSE,
                 _("Document ended unexpectedly while inside an attribute value"));
      break;
    ...
    case STATE_INSIDE_PASSTHROUGH:
      set_error (context, error, G_MARKUP_ERROR_PARSE,
                 _("Document ended unexpectedly inside a comment or processing instruction"));
      break;
