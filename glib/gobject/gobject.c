  ...
  while (n--)
    {
      ...
      if (!class_pspec)
	{
	  g_critical ("Object class %s doesn't implement property '%s' from interface '%s'",
...
#define SUBSET(a,b,mask) (((a) & ~(b) & (mask)) == 0)
	...
	{
	  g_critical ("Flags for property '%s' on class '%s' are not compatible with the property on interface '%s'\n",
		      pspecs[n]->name,
		      g_type_name (G_OBJECT_CLASS_TYPE (class)),
		      g_type_name (iface_type));
	}
			 ...
			 GConnectFlags connect_flags)
{
  ...
  if (gobject)
	...
	closure = g_cclosure_new_object_swap(c_handler, gobject);
      } else {
        closure = g_cclosure_new_object(c_handler, gobject);
      }
#endif
