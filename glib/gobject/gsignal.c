...
#ifdef VAXC
#define G_STRLOC2 "VAXC"
#else
#define G_STRLOC2 G_STRLOC
#endif
			...
			GQuark   detail)
{
  ...
  if (node && detail && !(node->flags & G_SIGNAL_DETAILED))
    {
      g_warning ("%s: signal id `%u' does not support detail (%u)", G_STRLOC2, signal_id, detail);
      SIGNAL_UNLOCK ();
      return;
    }
  if (node && g_type_is_a (G_TYPE_FROM_INSTANCE (instance), node->itype))
    ...
    g_warning ("%s: signal id `%u' is invalid for instance `%p'", G_STRLOC2, signal_id, instance);
  SIGNAL_UNLOCK ();
}
			    ...
			    GDestroyNotify      data_destroy)
{
  ...
  if (!node || node->destroyed)
    {
      g_warning ("%s: invalid signal id `%u'", G_STRLOC2, signal_id);
      SIGNAL_UNLOCK ();
      return 0;
    }
  if (node->flags & G_SIGNAL_NO_HOOKS) 
    {
      g_warning ("%s: signal id `%u' does not support emission hooks (G_SIGNAL_NO_HOOKS flag set)", G_STRLOC2, signal_id);
      SIGNAL_UNLOCK ();
      return 0;
    }
			       ...
			       gulong hook_id)
{
  ...
  if (!node || node->destroyed)
    ...
    g_warning ("%s: signal \"%s\" had no hook (%lu) to remove", G_STRLOC2, node->name, hook_id);
  SIGNAL_UNLOCK ();
}
...
#else
	if (force_quark) {
	  *detail_p = colon[2] ? (g_quark_from_string (colon + 2)) : 0;
	} else {
	  *detail_p = colon[2] ? (g_quark_try_string (colon + 2)) : 0;
	}
#endif
    }
  else
  ...
  if (signal_id)
    {
      ...
      if (detail && !(node->flags & G_SIGNAL_DETAILED))
	g_warning ("%s: signal `%s' does not support details", G_STRLOC2, detailed_signal);
      else if (!g_type_is_a (itype, node->itype))
	g_warning ("%s: signal `%s' is invalid for instance `%p'", G_STRLOC2, detailed_signal, instance);
      else
	{
	  Emission *emission_list = node->flags & G_SIGNAL_NO_RECURSE ? g_restart_emissions : g_recursive_emissions;
	  Emission *emission = emission_find (emission_list, signal_id, detail, instance);
...
#else
  g_strdelimit (name, "_-|> <.:^", '_');  /* FIXME do character checks like for types */
#endif
				 ...
				 GClosure *class_closure)
{
  ...
  if (!g_type_is_a (instance_type, node->itype))
    g_warning ("%s: type `%s' cannot be overridden for signal id `%u'", G_STRLOC2, type_debug_name (instance_type), signal_id);
  else
    {
      ClassClosure *cc = signal_find_class_closure (node, instance_type);
      
      if (cc && cc->instance_type == instance_type)
	g_warning ("%s: type `%s' is already overridden for signal id `%u'", G_STRLOC2, type_debug_name (instance_type), signal_id);
      else
				...
				GValue       *return_value)
{
  ...
  if (emission)
	...
	g_warning ("%s: signal id `%u' cannot be chained from current emission stage for instance `%p'", G_STRLOC2, node->signal_id, instance);
    }
  else
    g_warning ("%s: no signal is currently being emitted for instance `%p'", G_STRLOC2, instance);
  if (closure)
    {
...
#else
	  if (swapped) {
	    handler->closure = g_closure_ref (g_cclosure_new_swap (c_handler, data, destroy_data));
	  } else {
	    handler->closure = g_closure_ref (g_cclosure_new (c_handler, data, destroy_data));
	  }
#endif
                        ...
                        gulong   handler_id)
{
  ...
  if (handler)
    ...
    g_warning ("%s: instance `%p' has no handler with id `%lu'", G_STRLOC2, instance, handler_id);
  SIGNAL_UNLOCK ();
}
		       ...
		       ...)
{
  ...
  if (signal_id)
    ...
    g_warning ("%s: signal name `%s' is invalid for instance `%p'", G_STRLOC2, detailed_signal, instance);
}
