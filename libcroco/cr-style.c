...
num_prop_code_to_string (enum CRNumProp a_code)
{
        ...
        if (a_code >= len) {
                cr_utils_trace_info (
 "A field has been added to 'enum CRNumProp' and no matching entry has been added to gv_num_prop_dump_infos table.\nPlease add the missing matching entry");
                return NULL;
        }
        if (gv_num_props_dump_infos[a_code].code != a_code) {
                cr_utils_trace_info (
 "mismatch between the order of fields in 'enum CRNumProp' and the order of entries in the gv_num_prop_dump_infos table");
                return NULL;
        }
...
rgb_prop_code_to_string (enum CRRgbProp a_code)
{
        ...
        if (a_code >= len) {
                cr_utils_trace_info (
 "A field has been added to 'enum CRRgbProp' and no matching entry has been added to gv_rgb_prop_dump_infos table.\nPlease add the missing matching entry");
                return NULL;
        }
        if (gv_rgb_props_dump_infos[a_code].code != a_code) {
                cr_utils_trace_info (
 "mismatch between the order of fields in 'enum CRRgbProp' and the order of entries in the gv_rgb_props_dump_infos table");
                return NULL;
        }
...
border_style_prop_code_to_string (enum CRBorderStyleProp a_code)
{
        ...
        if (a_code >= len) {
                cr_utils_trace_info (
 "A field has been added to 'enum CRBorderStyleProp' and no matching entry has been added to gv_border_style_prop_dump_infos table.\nPlease add the missing matching entry");
                return NULL;
        }
        if (gv_border_style_props_dump_infos[a_code].code != a_code) {
                cr_utils_trace_info (
 "mismatch between the order of fields in 'enum CRBorderStyleProp' and the order of entries in the gv_border_style_props_dump_infos table");
                return NULL;
        }
