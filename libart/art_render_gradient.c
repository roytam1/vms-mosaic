				     ...
				     art_u8 *dest, int y)
{
  ...
  ArtGradientStop *fstops1 = NULL;
  ArtGradientStop *fstops2 = NULL;
  ...
  {
    extra_stops = 0;
    fstops1 = tmp_stops = stops = (ArtGradientStop *)calloc(1, sizeof(ArtGradientStop) * (n_stops + 2));
    if (gradient->stops[0].offset > EPSILON /* 0.0 */)
      {
  ...
  if (spread == ART_GRADIENT_REFLECT)
    {
      tmp_stops = stops;
      fstops2 = stops = (ArtGradientStop *)calloc(1, sizeof(ArtGradientStop) * n_stops * 2);
